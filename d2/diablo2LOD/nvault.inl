...
Save_Data(id)
{
	...
	for(new hero_id = 0; hero_id < MAX_CHARS + 1 ; hero_id++)
	{
		...
		g_Coins[id][hero_id], g_CoinsInv[id][hero_id], g_PlayerHero[id][hero_id], g_PlayerCharActive[id][hero_id], g_PlayerSecondHero[id][hero_id] )
	}	
...
Load_Data(id)
{
	...
	for(new hero_id = 0; hero_id < MAX_CHARS + 1 ; hero_id++)
	{
		...
		strtok(szClassLevel, szSecond, sizeof( szSecond ) - 1, szClassLevel, sizeof( szClassLevel ) - 1, ' ')
		...
		g_PlayerSecondHero[id][hero_id] = str_to_num(szSecond);
	}	
