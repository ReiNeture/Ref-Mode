...
public plugin_init() 
{
	...
	// 46 53
	g_SkillId = register_d2_skill(PLUGIN_NAME, "移動中被攻擊時,有一定機率可以迴避掉.", MAKO, Skill_Level, NOT_DISPLAY)
}
...
{
	if ( is_user_alive(victim) && get_p_hero(victim) == MAKO && get_p_skill( victim, g_SkillId ) > 0 && random_num( 0, 100 ) < AmazonEvade[ get_p_skill( victim, g_SkillId ) - 1 ] && IsPlayerMoving(victim) )
	{
		iDamage[0] = 0.0;
	}
}
