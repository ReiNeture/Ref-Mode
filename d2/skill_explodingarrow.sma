...
public plugin_init() 
{
	...
	//71 82
	g_SkillId = register_d2_skill(PLUGIN_NAME, "將爆炸屬性付在弓箭上.", ELEMENT, Skill_Level, DISPLAY)
...
{
	if ( get_p_hero(id) == ELEMENT && get_p_skill( id, g_SkillId ) > 0 && g_iCurSkill[id] == g_SkillId
...
{
	if ( get_p_hero(attacker) == ELEMENT && get_p_skill( attacker, g_SkillId ) > 0 && g_ShootWithFireArrow[attacker] )
	{
