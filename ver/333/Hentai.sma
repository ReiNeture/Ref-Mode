...
new const Monster_Models[MAX_MONSTERS][] =
{
	...
	"models/hgrunt.mdl","models/tentacle.mdl","models/babygarg.mdl","models/bigrat.mdl","models/gonome.mdl"
}
new const Monster_Xp[MAX_MONSTERS] =
{
	210,4000,190,120,0,20,0,120,0,50,0,0,0,0,700
}
new const Monster_Names[MAX_MONSTERS][] =
{
	"異型戰士","活動陰囊","瓦尼","首腦","巨人","食腦蟲","百募狗",
	"弗地崗人","聖甲蟲","松餅","人類戰士","鷹爪","小型巨人","老鼠","松餅聘骨"
}
new const Monster_Coin[MAX_MONSTERS] =
{
	52,1000,45,120,0,20,0,120,0,13,0,0,0,0,170
}
////////////////////////////////////////////////////////////////////New////////////////////////////////////////////////////////////////////
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	{
		client_print(attacker, print_center, "你殺了東西，獲得%d經驗值，外加10年有期徒刑", (0000 * get_cvar_num("Ako_exp")))
		semen[attacker][1] += 3000 * get_cvar_num("Ako_exp")
	}
	if(get_user_team(victim) == 1)
	{
		client_print(attacker, print_center, "你殺了花豹，獲得%d經驗值，外加動保協會的抗議", (400 * get_cvar_num("Ako_exp")))
		semen[attacker][1] += 400 * get_cvar_num("Ako_exp")
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	else if ( get_user_weapon(attacker) == CSW_KNIFE && get_user_team(attacker) == 1)
	{
        ...
        {
		client_print(attacker, print_center, "累積傷害達3000，獲得%d經驗值，附加獎勵%d經驗", (210 * get_cvar_num("Ako_exp")),(10-rein[attacker])*60)
...
public Monster_Killed(this, idattacker, shouldgib)
{
	...
	//	return HAM_IGNORED;
	if (get_boss_haken() == 1){
		client_print( idattacker, print_center, "目前不開放怪物，請於營業時間進入獵場，怪物也需要休息。")
		return HAM_IGNORED;
	}
	...
	for(new monsters = 0; monsters < MAX_MONSTERS; monsters++) 
	{
		if( equal( MonsterMdl, Monster_Models[monsters] ) ) 
		{
			...
			{
				new exp = (Monster_Xp[monsters] + ((10-rein[idattacker])*3)) * get_cvar_num("Ako_exp")
				new coin = (Monster_Coin[monsters] + random_num(10,30)) * get_cvar_num("Ako_exp")
				semen[idattacker][1] += exp
				g_money[idattacker] += coin
				client_print( idattacker, print_center, "你殺了 %s, +%d經驗, +%d精錢", Monster_Names[monsters], exp, coin);
				killmon_count[idattacker] ++
				make_entity(this, "LowBouns", "models/head.mdl",0,0,150)
			}
		}
	}
	if( equal( MonsterMdl, "models/big_mom.mdl" ) )
	{
		new s_name[32]
		get_user_name(idattacker, s_name, 31);
		client_printcolor(0, "/g[系統]/ctr%s砍掉了陰囊",s_name)
	}
...
{
	new Float:tot_damage,Float:porn_num
	porn_num = float(porn[attacker][5])
	tot_damage = damage + float(porn[attacker][0]) + random_float(porn_num,16.0) + float(normal_knife_damage[kfmod[attacker]])
	
	new Float:iArray[1]; iArray[0] = tot_damage;
	SetHamParamFloat(4, iArray[0] )

	set_hudmessage(255, 255, 255, 0.49, 0.3, 1, 1.5, 1.0, 0.0, 0.0, 2)
	show_hudmessage(attacker ,"+%d",floatround(iArray[0]));
}
...
public choose_level_maingun2(id, menu, item)
{
	...
	if(!g_has_handjob[id] && get_boss_haken() == 1)
	{
...
public rein_switch_menu(id)
{
	new menu[400], len
	len = 0
	len += formatex(menu[len], charsmax(menu) - len, "\w投胎需要達到\r800000經驗值\w及\r200000金^n^n")
...
public rein_menu(id, key)
{
	switch (key){
		case 0:{
			if (semen[id][0] >= 100 && rein[id]<10 && semen[id][1] >= 800000 && g_money[id] >= 200000){
				rein[id] ++
				semen[id][0]=1
				semen[id][1]=0
				porn[id][0]=0;porn[id][1]=0;porn[id][2]=0;porn[id][3]=0;porn[id][4]=0;porn[id][5]=0
				g_money[id] -= 200000
...
{
	if(!g_has_handjob[id] && get_boss_haken() == 1 && get_user_team(id) == 2)	
	{
		switch (key){
			case 0:{
...
public basic_weapon(id, menu, item)
{
	...
	switch(i)
	{
		case 1:
		{
			if (rein[id] < 3 && get_boss_haken() == 1 && !g_has_handjob[id])
			{
				switch(mun)
				{
					case 1: 
...
public buy_item_menu(id)
{
	...
	len += formatex(menu[len], charsmax(menu) - len, "\w2.衝擊結界(單場次)  COST:80隻蘿莉^n")
...
public show_skill_menu(id)
{
	...
	len += formatex(menu[len], charsmax(menu) - len, "\y5.對怪浮動傷害最低值   \r%d/%d   [最低浮動%d傷害]", porn[id][5],6+rein[id],porn[id][5])
...
public skill_menu(id, key)
{
	switch (key)
	{
		...
		case 4:
		{
			if (porn[id][4] >= 1 && porn[id][5] < 6+rein[id])
			{
				porn[id][5] ++
				porn[id][4] -- 
				show_skill_menu(id)
...
public ochinchinmenu(id)
{
	...
	len += formatex(menu[len], charsmax(menu) - len, "\y3.對怪浮動傷害最低值   \r%d^n^n", porn[id][5])
	len += formatex(menu[len], charsmax(menu) - len, "\y4.雞雞長度 \r%d^n^n", porn[id][3])
...
public show_hud(taskid)
{
	...
	show_hudmessage(taskid ,"|---------------------|^n|血量:%d|^n|等級:%d||經驗:%d/%d|^n|經驗倍率:%d|^n|投胎次數:%d|^n|蘿莉數量:%d|^n|金錢:%d|^n|---------------------|", get_user_health(taskid) ,semen[taskid][0], semen[taskid][1], dildo[semen[taskid][0]], get_cvar_num("Ako_exp"),rein[taskid],killmon_count[taskid],g_money[taskid])
	...
	{
		show_hudmessage(taskid , "|---------------------|^n|狀態:死亡|^n|等級:%d||經驗:%d/%d|^n|經驗倍率:%d|^n|投胎次數:%d|^n|蘿莉數量:%d|^n|金錢:%d|^n|---------------------|",semen[taskid][0], semen[taskid][1], dildo[semen[taskid][0]], get_cvar_num("Ako_exp"),rein[taskid],killmon_count[taskid],g_money[taskid])
	}
...
public SaveData2(id)
{
	...
	format(vaultkey, 63, "%s", name) 
	format(vaultdata, 255, "%i#%i#%i#%i#%i#%i#", rein[id],porn[id][5],norkf[id][0],norkf[id][1],norkf[id][2],g_money[id])
...
public LoadData2(id)
{
	...
	new re_in[32],po_rn[32],nkf0[32],nkf1[32],nkf2[32],gmoney[32]
	parse(vaultdata, re_in,31,po_rn,31,nkf0,31,nkf1,31,nkf2,31,gmoney,31)
	porn[id][5] = str_to_num(po_rn)
