...
// add double forward slash -or- comment to disable hud mod
// this is optional though, not a mandatory
#define _ENABLE_HUDMOD

#if defined _ENABLE_HUDMOD	
	#include <metadrawer>
	#include <md_csohud>
#endif
...
#define weapon_dgaxe "weapon_knife"

#define v_model "models/v_dgaxe.mdl"
#define v_model2 "models/v_dgaxe_2.mdl"
#define p_model "models/p_dgaxe_a.mdl"
#define p_model2 "models/p_dgaxe_b.mdl"
#define spr_wpn	"knife_dgaxe"
...
#define MODEL_SUMMON "models/dgaxe_summon.mdl"
#define SPR_SUMMON_SPAWN "sprites/ef_dgaxe_smoke.spr"
#define SPR_SUMMON_EXPLO "sprites/ef_dgaxe_explo.spr"

#define TGA_AIMON "sprites/dgaxe_aim_on.tga"
#define TGA_AIMON2 "sprites/dgaxe_aim_on_2.tga"
#define TGA_AIMOFF "sprites/dgaxe_aim_off.tga"
...
new g_had_dgaxe[33], g_rb, g_combo[33], g_flame[33], Float:g_SkillTimer[33], g_Ammo[33], g_ModeCheck[33], g_CoolingDown[33], g_Block[33]
...
public plugin_init() 
{
	...
	RegisterHam(Ham_Item_PostFrame, weapon_dgaxe, "fw_Item_PostFrame")
...
public md_init()
{
	md_loadimage(TGA_AIMON)
	md_loadimage(TGA_AIMON2)
	md_loadimage(TGA_AIMOFF)
}
#endif

public hook_rb(id)
{
	engclient_cmd(id, weapon_dgaxe)
	return PLUGIN_HANDLED
}

public zp_extra_item_selected(id, itemid) if(itemid == g_rb) get_dgaxe(id)
...
public get_dgaxe(id)
{
	...
	else engclient_cmd(id,weapon_dgaxe)
	
	#if defined _ENABLE_HUDMOD
	mdcsohud_regwpnhud(id, CSW_DGAXE, spr_wpn)
	#endif
	
	message_begin(MSG_ONE_UNRELIABLE, get_user_msgid("WeaponList"), _, id)
	write_string(g_had_dgaxe[id] == 1 ? spr_wpn : weapon_dgaxe)
	write_byte(2)
	write_byte(100)
	write_byte(-1)
	write_byte(-1)
...
public remove_crow9(id)
{
	...
	#if defined _ENABLE_HUDMOD
	mdcsohud_resetwpnhud(id, CSW_DGAXE)
	#endif
...
{
	if(!is_user_connected(id) || !is_user_alive(id) || !g_had_dgaxe[id])
	{
		#if defined _ENABLE_HUDMOD
	...
	#if defined _ENABLE_HUDMOD
	if(!(pev(id,pev_button) & IN_ATTACK) && IsOn(id))
		SetNewDrawing(id, (IsAimingAtVictim(id) && !g_CoolingDown[id]) ? TGA_AIMON : TGA_AIMOFF)
	#endif
}
...
public WeaponSkill(id, iEnt, iButton)
{
	...
	if(iButton & IN_ATTACK)
	{
		...
		#if defined _ENABLE_HUDMOD
		if(IsOn(id) && !g_CoolingDown[id]) SetNewDrawing(id, TGA_AIMON2)
		#endif
...
#if defined _ENABLE_HUDMOD
public SetNewDrawing(id, data[])
{
	md_removedrawing(id, 1, 20)
	md_drawimage(id, 20, 0, data, 0.5, 0.5, 1, 1, 255, 255, 255, 255, 0.0, 0.0, 0.0)
}
#endif
